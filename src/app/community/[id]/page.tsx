...
export default function ForumPostPage({ params }: { params: { id: string } }) {
  ...
  const handleAddReply = (data: ReplyFormValues) => {
    ...
    // Here we're updating the mock data directly.
    const updatedPost = { ...post, replies: [...post.replies, newReply] };
    const postIndex = forumPosts.findIndex(p => p.id === params.id);
    if (postIndex !== -1) {
      ...
      <Card>
        <CardHeader>
            ...
            <CardDescription className="flex items-center gap-4 pt-2">
                ...
                <span className="text-sm text-muted-foreground">{new Date(post.date).toLocaleString()}</span>
            </CardDescription>
        </CardHeader>
        <CardContent>
